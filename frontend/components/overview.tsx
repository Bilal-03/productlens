...
export function Overview() {
  const overview = useQuery({ queryKey: ["overview"], queryFn: () => api<OverviewResponse>("/analytics/overview", { method: "POST", body: JSON.stringify({ period: "last_30_days" }) }), staleTime: 300_000 });
  // Keep a small fallback for a temporarily unavailable extended overview;
  // normal loads use one aggregated request instead of six duplicate KPI calls.
  const fallbackMetrics = useQueries({ queries: metricNames.map((metric) => ({ queryKey: ["metric", metric], enabled: overview.isError, queryFn: () => api<{ metric: { label: string; format: string }; current: { value: number }[]; previous: { value: number }[]; current_period: { label: string } }>("/analytics/kpi", { method: "POST", body: JSON.stringify({ metric, period: metric === "d30_retention" ? "last_90_days" : "last_30_days" }) }) })) });
  const revenue = overview.data?.kpis.mrr ?? fallbackMetrics[3].data;
  ...
  return <>
    ...
    <div className="mb-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
      {metricNames.map((name, index) => { const payload = overview.data?.kpis[name] ?? fallbackMetrics[index].data; const current = payload?.current?.[0]?.value ?? 0; const previous = payload?.previous?.[0]?.value ?? 0; const unavailable = overview.isError && fallbackMetrics[index].isError; return <MetricCard key={name} label={payload?.metric?.label ?? name.replaceAll("_"," ")} value={unavailable ? "Unavailable" : payload ? format(current,payload.metric?.format ?? "integer") : "—"} delta={payload && previous ? (current-previous)/Math.abs(previous) : null} note={unavailable ? "Database may be paused" : payload?.current_period?.label ?? "Loading governed metric"} loading={overview.isLoading || (!overview.data && fallbackMetrics[index].isLoading)}/>; })}
    </div>
