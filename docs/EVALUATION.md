...
The first P3 access-boundary suite additionally tests signed assertion round trips, tamper/expiry/future-issue rejection, explicit role permissions, workspace/subject session isolation, anonymous compatibility, the access-context contract, `401` invalid-token handling, and `403` notebook-write enforcement. It does not claim enterprise SSO or tenant-level source-data isolation; those require an identity-provider and connector-backed integration environment.
