...
The first P3 access-boundary suite additionally tests signed assertion round trips, tamper/expiry/future-issue rejection, explicit role permissions, workspace/subject session isolation, anonymous compatibility, the access-context contract, `401` invalid-token handling, and `403` notebook-write enforcement. The OIDC slice adds RSA-signed bearer-token checks for issuer, audience, required expiry, key-id rotation, exact group mapping, RBAC, and tenant context. It does not claim enterprise SSO provisioning or tenant-level source-data isolation; those require an identity-provider deployment and connector-backed integration environment.
