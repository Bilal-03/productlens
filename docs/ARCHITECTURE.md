...
The overview's 90-day revenue and signup charts use the same bounded daily-series compiler and roll up the grouped rows in memory, rather than issuing one database round trip per weekly bucket. A bounded warm-process cache reuses tenant-bound connector engines, dataset fingerprints, connector contract status, and completed result payloads; the operational result cache remains the cross-process source of truth. Fingerprints are cached for only a short configured interval (`DATASET_VERSION_CACHE_SECONDS`, default five seconds) so refreshes are faster without turning streaming updates into a permanent stale snapshot.
