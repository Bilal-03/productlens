...
The overview's first paint uses a dedicated bounded summary endpoint for the six headline cards. It computes current values without comparison fan-out, tolerates an individual slow metric with a warning, and then the frontend requests the below-the-fold charts, funnel, acquisition, and retention details. Unfiltered MAU reads the UTC daily-activity rollup; the 90-day revenue and signup charts use the same bounded daily-series compiler and roll up grouped rows in memory, rather than issuing one database round trip per weekly bucket. A bounded warm-process cache reuses tenant-bound connector engines, dataset fingerprints, connector contract status, and completed result payloads; the operational result cache remains the cross-process source of truth. Fingerprints are cached for only a short configured interval (`DATASET_VERSION_CACHE_SECONDS`, default five seconds) so refreshes are faster without turning streaming updates into a permanently stale snapshot.
