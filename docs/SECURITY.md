...
- OIDC deployment variables (`OIDC_ISSUER_URL`, `OIDC_AUDIENCE`, `OIDC_JWKS_URL`, `OIDC_WORKSPACE_CLAIM`, `OIDC_GROUPS_CLAIM`, and `OIDC_ROLE_GROUPS`) are server-only. The frontend stores only the short-lived bearer assertion and never receives JWKS or client secrets.
