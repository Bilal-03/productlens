...
- Warm-process connector, fingerprint, status, and result caches are bounded and keyed by the tenant/source boundary (and operational database identity where applicable). The persistent operational result cache remains the cross-process source of truth; no browser cache or client-provided source identifier is trusted for analytics data.
